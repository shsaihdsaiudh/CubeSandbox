...
BUILD_CONTEXT_DIR="${CUBE_LCM_DIR}/build-context"
COMPOSE_TEMPLATE="${CUBE_LCM_DIR}/docker-compose.yaml.template"
COMPOSE_FILE="${CUBE_LCM_DIR}/docker-compose.yaml"

CUBE_LCM_IMAGE_TAG="${CUBE_LCM_IMAGE_TAG:-cube-lifecycle-manager:one-click}"
...
render_template_atomic \
  "${COMPOSE_TEMPLATE}" \
  "${COMPOSE_FILE}" \
  -e "s#__CUBE_LCM_IMAGE__#$(escape_sed "${CUBE_LCM_IMAGE_TAG}" '#')#g" \
  -e "s#__CUBE_LCM_CONTAINER_NAME__#$(escape_sed "${CUBE_LCM_CONTAINER_NAME}" '#')#g" \
  -e "s#__CUBE_LCM_BUILD_CONTEXT__#$(escape_sed "${BUILD_CONTEXT_DIR}" '#')#g" \
...
cube_lcm_compose_run build cube-lifecycle-manager
